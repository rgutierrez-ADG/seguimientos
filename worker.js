...
export default {
  async fetch(req, env, ctx) {
    ...
    if (apiPath==='/rebuild-att-meta' && m==='POST') return await rebuildAttMeta(req,env);
...
async function stateGet(req, env) {
  ...
  // Leer state y attMeta en paralelo — 2 llamadas KV en vez de N
  const [raw, metaRaw] = await Promise.all([
    env.SGCIA_KV.get('state'),
    env.SGCIA_KV.get('att_meta'),
  ]);
  const state = raw ? JSON.parse(raw) : { treatments:{}, descargas:[] };
  // Si att_meta no existe aún, reconstruirlo automáticamente (migración única)
  let attMeta = {};
  if (metaRaw) {
    attMeta = JSON.parse(metaRaw);
  } else {
    // Primera vez: reconstruir el índice leyendo las claves att:*
    // Esto solo ocurre una vez — después siempre hay att_meta
    const meta = {};
    let cursor;
    do {
      const r = await env.SGCIA_KV.list({ prefix:'att:', cursor });
      for (const k of r.keys) {
        if (k.name === 'att_meta') continue;
        const v = await env.SGCIA_KV.get(k.name);
        if (v) {
          try {
            ...
            if (Array.isArray(arr) && arr.length > 0)
              meta[rk] = { count:arr.length, totalSize:arr.reduce((a,f)=>a+(f.size||0),0) };
          } catch(e){}
        }
      }
      cursor = r.list_complete ? null : r.cursor;
    } while (cursor);
    // Guardar para que las próximas llamadas sean instantáneas
    await env.SGCIA_KV.put('att_meta', JSON.stringify(meta));
    attMeta = meta;
  }
...
async function attPut(req, env) {
  ...
  // Guardar adjuntos y actualizar metadata en paralelo
  const metaRaw = await env.SGCIA_KV.get('att_meta');
  const meta    = metaRaw ? JSON.parse(metaRaw) : {};
  if (files.length > 0) {
    meta[key] = { count: files.length, totalSize: files.reduce((a,f)=>a+(f.size||0),0) };
  } else {
    delete meta[key];
  }
  await Promise.all([
    env.SGCIA_KV.put('att:'+key, JSON.stringify(files)),
    env.SGCIA_KV.put('att_meta', JSON.stringify(meta)),
  ]);
  return j({ ok:true });
}
...
// ── /api/rebuild-att-meta ─────────────────────────────────────────────────
// Reconstruye el índice att_meta leyendo todas las claves att:* una sola vez
// Solo necesita ejecutarse una vez después del deploy
async function rebuildAttMeta(req, env) {
  await requireSession(req, env);
  const meta = {};
  let cursor;
  do {
  ...
  await env.SGCIA_KV.put('att_meta', JSON.stringify(meta));
  return j({ ok:true, rebuilt: Object.keys(meta).length });
}
